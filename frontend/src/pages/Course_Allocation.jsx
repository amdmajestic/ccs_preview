...
class Course_Allocation extends Component {
  ...
  async componentDidMount() {
    ...
    await this.initCourseAllocation();
  }

  async initCourseAllocation() {
    await api
      .get("/course_allocation/lectures/")
      .then((response) => {
        this.setState({
  ...
  getUniqueSemesters = (lectures) => {
    ...
    const semesters = lectures.map(
      (lecture) => lecture.lec_class_id.class_semester_id.id
    );
  ...
  getUniqueSections = (semesterLectures) => {
    const sections = semesterLectures.map(
      (semesterLecture) => semesterLecture.lec_class_id.class_section_id.name
    );
    // convert the Set object back to an array
  ...
  renderTables = () => {
    ...
    return uniqueSemesters.map((semester_no, index) => {
      ...
      if (semesterLectures) {
        ...
        const lectureCourses = semesterLectures.filter(
          (lecture) =>
            lecture.lec_class_id.class_section_id.name === uniqueSections[0]
        );
              ...
              <table className="table-auto min-w-full border border-collapse border-black font-sans font-normal max-sm:text-xs sm:text-sm md:text-md lg:text-lg whitespace-nowrap">
                <thead className="bg-sky-400 font-bold">
                  <tr>
                    <th
                      ...
                      colSpan={8}
                      className="border border-black py-1 font-extrabold max-sm:text-lg sm:text-xl md:text-2xl lg:text-3xl"
                    >
                  ...
                  {hec_rev_crc && (
                    <tr>
                      <th
                        colSpan={8}
                        className="border border-black font-thin font-mono"
                      >
                        ({hec_rev_crc})
                      </th>
                    </tr>
                  ...
                  <tr>
                    <th className="border border-black font-semibold bg-gray-400">
                      Course
                    </th>
                    <th className="border border-black font-semibold bg-gray-400">
                      Courses
                    ...
                    <th
                      colSpan={uniqueSections.length}
                      className="border border-black font-semibold bg-gray-400"
                    >
                      Section
                    </th>
                  </tr>
                ...
                <tbody>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="border border-black px-4 py-2 bg-gray-300">
                            {lecture.lec_course_id.crs_coordinator_id?.user
                              .name || "N/A"}
                          </td>
                          {Array.from({
                            length:
                              lecture.lec_class_id.class_semester_id
                                .has_sections,
                            ...
                            >
                              {lecture.lec_instr_id
                                ? lecture.lec_instr_id.user.name
                  ...
                  <tr>
                    <td
                      colSpan="2"
                      className="border border-black bg-gray-400 pl-3 p-2 font-bold"
                    >
                      Total Credits
                    </td>
                    <td
                      colSpan="1"
                      className="border border-black p-2 text-center font-bold bg-gray-400"
                    >
                      {totalCreditHrs}
                    </td>
                  ...
                  {index === 0 && (
                    <>
                      <tr>
                        <td
                          colSpan="8"
                          className="border border-none py-4 text-center font-bold"
                        ></td>
                      </tr>
                      <tr>
                        <td
                          colSpan="2"
                          className="border border-black border-t-gray-500 border-r-gray-500 px-4 py-2 bg-green-600 "
                        >
                          {" "}
                          DeficencyCourse
                        </td>
                      ...
                      <tr className="bg-green-100">
                        ...
                        <td
                          colSpan={uniqueSections.length}
                          className="border border-black px-4 py-2 bg-green-100"
                        >
                          Ms. Asma Parveen
                        </td>
                      </tr>
    ...
    return (
      <>
        <div className="px-5 flex flex-col font-bold dark:text-white">
          ...
          <span className="text-left lg:text-sm py-2">Ver 1.0</span>
        </div>
        <div className="p-4 bg-grey-100 border border-sky-50 mt-2 min-h-screen">
          {loading ? (
            <div>
              <p className="text-center text-blue-500 mb-5 scale-125">
                Loading...
              </p>
            ...
            <div className="text-center">
              <p className="text-red-500 mb-3">
                No Course Allocation Schedule Created Yet!
              </p>
              <button
                onClick={this.initCourseAllocation}
